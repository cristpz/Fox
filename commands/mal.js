...
module.exports.run = async (bot, msg, [term]) => {
  ...
  request(url + term, function(err, res, body) {
    ...
    var x = 0;
    var info = new Object();
    info.aStats = new Object();
    info.mStats = new Object();

    do {
      var z = text[x].trim();
      var y = text[x + 1] ? text[x + 1].trim() : "";
      var zed = text[x + 2] ? text[x + 2].trim() : "";
      if (z + y + zed === "404NotFound") {
        return msg.channel.send(
          "Whoops! Looks like a user by that name does not exist."
        );
      } else if (z.length > 1) {
        if (z.startsWith("Online")) {
          if (zed.startsWith("ago")) {
            info.status = z.slice(6) + " " + y + " ago";
          } else if (z.startsWith("OnlineNow")) {
            info.status = z.slice(6, z.search("Gender"));
          } else if (zed.includes(":")) {
            info.status = z.slice(6) + " " + y + " " + new Date().getFullYear();
          } else if (
            z.includes("Yesterday") ||
            (z.includes("Today") && zed.slice(1, 2) === "M")
          ) {
            info.status = z.slice(6, -1) + " at " + y + zed.slice(0, 2);
          } else if (isNaN(zed) === false) {
            info.status = z.slice(6) + " " + y + " " + zed;
          }
        }
        ...
        if (z.includes("Gender")) {
          ...
          if (z.startsWith("ago")) {
            var amount2 = 9;
          } else if (z.startsWith("AM") || z.startsWith("PM")) {
            var amount2 = 8;
          } else {
            var amount2 = 15;
          }
        ...
        } else if (z === "All" && !info.friends) {
          info.friends = y.slice(1, -8);
    ...
    var list = [];
    if (info.gender) {
      list.push("🚻 Gender: " + info.gender);
    }
    if (info.birthday) {
      list.push("🎂 Birthday: " + info.birthday);
    }
    if (info.friends) {
      list.push("👫 Friends: " + info.friends);
    }
    ...
    if (list.length > 0) {
      embed.addField("__General:__", list.join("\n"));
    }
    embed
      .addField(
        "__Anime:__",
